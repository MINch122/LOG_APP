...
2. Add `event_maker` to your `targets.cmake`.
3. Add to `cfe_es_startup.scr`:
   ```
   CFE_APP, event_maker, EVENT_MAKER_Main, EVENT_MAKER, 80, 16384, 0x0, 0;
   ```

## Customization

- **Message IDs**: Edit `fsw/platform_inc/event_maker_msgids.h`
  (or `config/` defaults for Draco+)
- **Performance ID**: Edit the perfids header
- **Table structure**: Edit the table header and `fsw/tables/event_maker_tbl.c`
