...
void Test_LOG_TaskPipe(void)
{
    ...
    UT_CHECKEVENT_SETUP(&EventTest, LOG_MID_ERR_EID, "LOG: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(LOG_CMD_MID);
    FcnCode   = LOG_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    LOG_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(LOG_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    LOG_TaskPipe(&TestMsg.SBBuf);
...
void Test_LOG_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void LOG_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        LOG_NoopCmd_t          Noop;
        LOG_ResetCountersCmd_t Reset;
        LOG_ProcessCmd_t       Process;
        LOG_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, LOG_CMD_LEN_ERR_EID,
    ...
    LOG_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(LOG_NoopCmd, 1);
    ...
    UtAssert_STUB_COUNT(LOG_ResetCountersCmd, 1);
    ...
    /* note this will end up calling LOG_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = LOG_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(LOG_ProcessCmd, 1);
    ...
    UtAssert_STUB_COUNT(LOG_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, LOG_CC_ERR_EID, "Invalid ground command code: CC = %d");
    LOG_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_LOG_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool LOG_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    LOG_VerifyCmdLength(NULL, size + 1);
...
{
    ADD_TEST(LOG_TaskPipe);
    ADD_TEST(LOG_ProcessGroundCommand);
    ADD_TEST(LOG_VerifyCmdLength);
}
