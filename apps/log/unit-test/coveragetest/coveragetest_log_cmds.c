...
**
** Purpose:
** Coverage Unit Test cases for the LOG Application
**
** Notes:
...
void Test_LOG_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void LOG_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    ...
    UtAssert_ADDRESS_EQ(MsgSend, &LOG_Data.HkTlm);

    /* Confirm timestamp msg address */
    UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
    UtAssert_ADDRESS_EQ(MsgTimestamp, &LOG_Data.HkTlm);
...
void Test_LOG_NoopCmd(void)
{
    /*
     * Test Case For:
     * void LOG_NoopCmd( const LOG_Noop_t *Msg )
     */
    LOG_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CHECKEVENT_SETUP(&EventTest, LOG_NOOP_INF_EID, NULL);

    UtAssert_INT32_EQ(LOG_NoopCmd(&TestMsg), CFE_SUCCESS);
...
void Test_LOG_ResetCountersCmd(void)
{
    /*
     * Test Case For:
     * void LOG_ResetCounters( const LOG_ResetCounters_t *Msg )
     */
    LOG_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, LOG_RESET_INF_EID, "LOG: RESET command");

    UtAssert_INT32_EQ(LOG_ResetCountersCmd(&TestMsg), CFE_SUCCESS);
...
void Test_LOG_ProcessCmd(void)
{
    /*
     * Test Case For:
     * void  LOG_ProcessCmd( const LOG_ProcessCmd_t *Msg )
     */
    LOG_ProcessCmd_t   TestMsg;
    ...
    /* Provide some table data for the LOG_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UtAssert_INT32_EQ(LOG_ProcessCmd(&TestMsg), CFE_SUCCESS);
    ...
    UtAssert_INT32_EQ(LOG_ProcessCmd(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
    ...
    UtAssert_INT32_EQ(LOG_ProcessCmd(&TestMsg), CFE_TBL_ERR_NO_ACCESS);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 4);
}

void Test_LOG_DisplayParamCmd(void)
{
    /*
     * Test Case For:
     * void  LOG_DisplayParamCmd( const LOG_DisplayParamCmd_t *Msg )
     */
    LOG_DisplayParamCmd_t TestMsg;
    UT_CheckEvent_t              EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, LOG_VALUE_INF_EID, "LOG: ValU32=%lu, ValI16=%d, ValStr=%s");
    TestMsg.Payload.ValU32 = 10;
    TestMsg.Payload.ValI16 = -4;
    snprintf(TestMsg.Payload.ValStr, sizeof(TestMsg.Payload.ValStr), "Hello");

    UtAssert_INT32_EQ(LOG_DisplayParamCmd(&TestMsg), CFE_SUCCESS);
    /*
     * Confirm that the event was generated
     */
    UtAssert_UINT32_EQ(EventTest.MatchCount, 1);
...
{
    ADD_TEST(LOG_ReportHousekeeping);
    ADD_TEST(LOG_NoopCmd);
    ADD_TEST(LOG_ResetCountersCmd);
    ADD_TEST(LOG_ProcessCmd);
    ADD_TEST(LOG_DisplayParamCmd);
}
