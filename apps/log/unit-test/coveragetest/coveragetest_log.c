...
void Test_LOG_Main(void)
{
    ...
    /*
     * LOG_Main does not return a value,
     ...
     * The first is for LOG_Init().  As this is in the same
     ...
     * the LOG_Init() failure path was taken.
     */
    LOG_Main();
     ...
     */
    UtAssert_UINT32_EQ(LOG_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, LOG_PIPE_ERR_EID, "LOG: SB Pipe Read Error, App Will Exit");
...
void Test_LOG_Init(void)
{
    ...
    UtAssert_INT32_EQ(LOG_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(LOG_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(LOG_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(LOG_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
